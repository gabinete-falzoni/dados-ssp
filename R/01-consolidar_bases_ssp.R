# Fonte dos dados:
# 2016-2021: Pedido de LAI. Extração:
# 1) Base de dados com informações extraídas pelo Sistema de Registro Digital de Ocorrências (RDO)
# 2) Pesquisa por Data do Registro e Boletins Principais por Rubrica tentada e consumada.
# 3) Ocorrências no Estado de São Paulo:
# 4) Rubricas de pesquisa por:  - Furto (art. 155 do Código Penal)   - Roubo (art. 157 do Código Penal).
# 5) Objeto subtraído;
# 6) Tipo de objeto por Bicicleta;
# 7) Período: de 01 de janeiro 2016 a 30 de setembro de 2022.
#
# 2022-2024: Site da SSP-SP
# https://www.ssp.sp.gov.br/estatistica/consultas
# Base de Objetos subtraídos
...
antigos <- sprintf('%s/originais/Dados brutos 2016-2021.xlsx', pasta_dados)
antigos <- read_xlsx(antigos, col_types = 'text')
head(antigos)
...
antigos <-
  ...
  mutate(ANO_BO = as.integer(ANO_BO),
         ...
         HORA_OCORRENCIA_BO = as.numeric(HORA_OCORRENCIA_BO),
         HORA_OCORRENCIA_BO = as.Date(HORA_OCORRENCIA_BO - 2, origin = "1900-01-01"),

         DATAHORA_REGISTRO_BO = as.numeric(DATAHORA_REGISTRO_BO),
         DATAHORA_REGISTRO_BO = as.POSIXct((as.numeric(DATAHORA_REGISTRO_BO) - 2) * 86400, origin = "1900-01-01", tz = "UTC"),

         DATAHORA_IMPRESSAO_BO = as_datetime(DATAHORA_IMPRESSAO_BO),
         ...
         HORA_OCORRENCIA_BO = as.numeric(HORA_OCORRENCIA_BO) * 24,
         HORA_OCORRENCIA_BO = format(as.POSIXct(HORA_OCORRENCIA_BO * 3600, origin = "1970-01-01", tz = "UTC"), "%H:%M:%S"),
         ...
         ANO_REGISTRO_BO = as.integer(ANO_REGISTRO_BO),
         MES_REGISTRO_BO = as.integer(MÊS_REGISTRO_BO),
...
# Arquivos 2022
ssp_2022 <- sprintf('%s/originais/ObjetosSubtraidos_2022.xlsx', pasta_dados)
...
# Arquivos 2023
ssp_2023 <- sprintf('%s/originais/ObjetosSubtraidos_2023.xlsx', pasta_dados)
...
# Arquivos 2024
ssp_2024 <- sprintf('%s/originais/ObjetosSubtraidos_2024.xlsx', pasta_dados)
...
ssp <-
  ...
  mutate(ANO_BO = as.integer(ANO_BO),
         ...
         HORA_OCORRENCIA_BO = as.numeric(DATAHORA_REGISTRO_BO),
         HORA_OCORRENCIA_BO = as.Date(DATAHORA_REGISTRO_BO - 2, origin = "1900-01-01"),
